  ...
  return (
    <article className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
      <div className="flex items-start justify-between gap-3">
        ...
        {applied && (
          <span className="rounded-full border border-teal-300 bg-teal-50 px-2.5 py-1 text-xs font-bold text-teal-700">
            {isEvent ? 'Applied' : 'Interested'}
          </span>
        )}
      </div>
      ...
      <div className="mt-4 flex flex-wrap gap-2">
        ...
        >
          {applied ? (isEvent ? 'Already Applied' : 'Already Interested') : primaryLabel || 'Show Interest'}
        </button>
        {onDetails && (
          <button
            type="button"
        ...
        {currentItems.map((item) => {
          ...
          return (
            <Card
              ...
              item={item}
              applied={applied}
              primaryLabel={type === 'stories' ? 'Read Story' : (item.type === 'EVENT' ? 'Apply' : (item.type === 'products' ? 'Open Product' : 'Interest'))}
          ...
          <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-2xl" onClick={(event) => event.stopPropagation()}>
            {selectedOpportunity.type === 'EVENT' ? (
              <form onSubmit={submitApplication} className="space-y-4">
                <p className="text-[11px] font-bold uppercase tracking-wide" style={{ color: TEAL }}>Apply for Event</p>
                <h3 className="mt-1 text-lg font-black text-slate-900">{selectedOpportunity.title}</h3>
                <p className="mt-1 text-sm text-slate-500">By: {selectedOpportunity.org}</p>
                <div>
                  <label className="mb-1 block text-xs font-bold text-slate-600" htmlFor="eventNote">Additional Notes (optional)</label>
                  <textarea
                    id="eventNote"
                    rows={4}
                ...
                <div className="flex gap-2">
                  ...
                  <button type="submit" disabled={applying} className="rounded-lg px-3 py-2 text-xs font-bold text-white disabled:opacity-60" style={{ backgroundColor: TEAL }}>
