...
const NAV_ICON_MAP={Profile:'user',Products:'box',Jobs:'orders',Events:'calendar',Orders:'orders',Payouts:'money'}
...
const TOP_LINKS=[
  {label:'Profile',to:'/startup/profile'},
  {label:'Products',to:'/startup/products'},
  {label:'Jobs',to:'/startup/jobs'},
  {label:'Events',to:'/startup/events'},
]
const DROPDOWN_GROUPS=[
  {label:'Marketplace',items:[{label:'Orders',to:'/startup/orders'},{label:'Payouts',to:'/startup/payouts'}]},
]
...
function NavDropdown({group,currentPath}){
  ...
  return(
    <div ref={ref} style={{position:'relative'}}>
      ...
      {open&&(<div style={{position:'absolute',top:'calc(100% + 8px)',left:-8,background:'#fff',borderRadius:14,border:'1px solid #f1f5f9',boxShadow:'0 16px 48px rgba(0,0,0,.10)',minWidth:220,padding:6,zIndex:1000,animation:'stDropIn .15s ease'}}>
...
export default function StartupWorkspaceLayout(){
  ...
  const userName=user?.name||user?.email||'Startup Admin'
  const initials=userName.split(' ').map(w=>w[0]).join('').slice(0,2).toUpperCase()

  const ALL_NAV=[
    {to:'/startup/profile',label:'Profile',icon:'user'},
    ...
    {to:'/startup/payouts',label:'Payouts',icon:'money'},
  ]

  return(
    <div style={{minHeight:'100vh',background:'#fff',fontFamily:"'Inter', sans-serif"}}>
      <style>{`@keyframes stDropIn{from{opacity:0;transform:translateY(-6px)}to{opacity:1;transform:translateY(0)}}.st-topnav{display:flex;align-items:center;width:100%;background:#fff;border-bottom:1.5px solid #f1f5f9;padding:0 28px;height:56px;position:sticky;top:0;z-index:100;gap:0}.st-nav-center{display:flex;align-items:center;gap:28px;margin-left:32px}.st-nav-search{display:flex;align-items:center;flex:1;justify-content:center;margin:0 24px}.st-nav-right{display:flex;align-items:center;gap:16px;margin-left:auto;flex-shrink:0}.st-hamburger{display:none}@media(max-width:900px){.st-nav-center{display:none!important}.st-nav-search{display:none!important}.st-hamburger{display:flex!important}}`}</style>
      ...
      <nav className="st-topnav">
        ...
        <button className="st-hamburger" onClick={()=>setMobileOpen(true)} style={{background:'none',border:'none',cursor:'pointer',padding:6,marginLeft:'auto'}}><Ic name="menu" size={22} color="#374151"/></button>
        <div className="st-nav-right">
          ...
          <div style={{display:'flex',alignItems:'center',gap:10,cursor:'pointer'}} onClick={()=>navigate('/startup/profile')}>
            <div style={{textAlign:'right'}}><span style={{display:'block',fontSize:12,fontWeight:700,color:NAVY,lineHeight:1.2}}>Startup</span><span style={{display:'block',fontSize:11,color:'#94a3b8',lineHeight:1.2}}>Profile</span></div>
      ...
      <div style={{position:'fixed',top:0,left:0,bottom:0,width:290,maxWidth:'82vw',background:'#fff',zIndex:9999,transform:mobileOpen?'translateX(0)':'translateX(-100%)',transition:'transform .25s cubic-bezier(.4,0,.2,1)',boxShadow:mobileOpen?'4px 0 30px rgba(0,0,0,.15)':'none',display:'flex',flexDirection:'column',overflowY:'auto'}}>
        ...
        <div style={{padding:'12px 20px',borderBottom:'1px solid #f8fafc'}}><div style={{display:'inline-flex',alignItems:'center',gap:6,padding:'5px 12px',borderRadius:20,background:`${GREEN}10`,border:`1px solid ${GREEN}30`}}><span style={{width:7,height:7,borderRadius:'50%',background:GREEN,display:'inline-block'}}/><span style={{fontSize:11,fontWeight:700,color:GREEN}}>Startup Workspace</span></div></div>
        <nav style={{flex:1,padding:'4px 12px'}}>
          {ALL_NAV.map(item=>{const a=currentPath.startsWith(item.to);return(<NavLink key={item.to} to={item.to} onClick={()=>setMobileOpen(false)} style={{display:'flex',alignItems:'center',gap:10,padding:'10px 12px',borderRadius:10,textDecoration:'none',marginBottom:1,background:a?'#f0fdf4':'transparent',color:a?GREEN:'#374151',fontSize:13.5,fontWeight:a?700:500,borderLeft:a?`3px solid ${GREEN}`:'3px solid transparent'}}><Ic name={item.icon} size={16} color={a?GREEN:'#64748b'}/>{item.label}</NavLink>)})}
        </nav>
