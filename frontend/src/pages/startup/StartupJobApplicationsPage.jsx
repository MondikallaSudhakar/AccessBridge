...
function ApplicationCard({ app, onStatusChange }) {
  ...
  const [note, setNote] = useState(app.startupReviewNote || '')
  ...
  const save = async (newStatus) => {
    ...
    try {
      ...
      const res = await fetch(`${API}/startups/jobs/applications/${app.id}/status?${params}`, {
        method: 'PATCH',
...
export default function StartupJobApplicationsPage({ jobId, jobTitle, startupId }) {
  ...
  useEffect(() => {
    if (!jobId) return
    setLoading(true)
    fetch(`${API}/startups/${startupId}/jobs/${jobId}/applications`, {
      headers: { Authorization: `Bearer ${localStorage.getItem('token')}` },
    })
      .then((r) => r.ok ? r.json() : [])
