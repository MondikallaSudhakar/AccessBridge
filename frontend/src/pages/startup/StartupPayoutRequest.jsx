...
export default function StartupPayoutRequest() {
  const { user } = useAuth()
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        const startupData = await api.get(`/startups/email/${encodeURIComponent(user.email)}`)
        if (!alive) return
  ...
  const submit = async (e) => {
    ...
    try {
      const created = await api.post(`/startups/${startup.id}/payout-requests`, { amount, notes: form.note.trim() })
