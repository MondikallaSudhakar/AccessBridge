    ...
    <div className="space-y-6" style={{ fontFamily: "'Inter', sans-serif" }}>
      <div className="mb-8 flex items-start justify-between gap-4">
        <div>
          ...
          <h1 className="text-3xl font-black text-slate-900">Job Postings</h1>
          <p className="text-sm text-slate-500 mt-1">Post inclusive job opportunities and review applicants from the community.</p>
        </div>
      </div>
      ...
      <div className="rounded-2xl border border-slate-200 bg-white shadow-sm overflow-hidden">
        <div className="flex items-center justify-between gap-3 border-b border-slate-100 px-5 py-4">
          <div>
            <h2 className="text-base font-extrabold text-slate-900">Posted Jobs</h2>
          ...
          <div className="flex gap-2">
            {!subscription?.active && (
              <button type="button" onClick={() => startSubscription({ onActivated: load })} className="rounded-lg px-4 py-2 text-xs font-bold text-white" style={{ backgroundColor: COLORS.success }}>
                Pay with Razorpay
              </button>
            )}
            <button
      ...
      {showForm && (
        <div className="border-b border-emerald-100 bg-emerald-50 px-5 py-5 space-y-3">
            ...
            <div key={job.id} className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
              <div className="flex flex-wrap items-start justify-between gap-3">
                ...
                <div className="flex flex-wrap gap-2">
                  <button type="button" onClick={() => setSelectedJob(job)} className="rounded-lg px-3 py-1.5 text-xs font-bold text-white" style={{ backgroundColor: COLORS.success }}>
                    View Applications
                  </button>
                  {job.status === 'OPEN' && (
                    <button type="button" onClick={() => closeJob(job.id)} className="rounded-lg border border-rose-200 bg-rose-50 px-3 py-1.5 text-xs font-bold text-rose-700 hover:bg-rose-100">
                      Close Job
                    </button>
                  )}
                </div>
