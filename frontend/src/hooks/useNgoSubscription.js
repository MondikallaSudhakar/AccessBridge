...
export function useNgoSubscription(ngoId) {
  const [loading, setLoading] = useState(Boolean(ngoId))
  ...
  const refreshSubscription = useCallback(async () => {
    ...
    setLoading(true)
    try {
      const data = await api.get(`/ngos/${ngoId}/subscription`)
  ...
  const startSubscription = useCallback(async ({ onActivated } = {}) => {
    if (!ngoId) {
      throw new Error('NGO profile is still loading.')
    }

    const order = await api.post(`/ngos/${ngoId}/subscription/order`, {})
